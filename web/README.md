...
- **First run**: The app builds an in-memory index from `data/processed/descriptions_text_by_source.jsonl` (tags are stored in each record's `tags` field); this can take 1–2 minutes. The page will show “Building index…” until then.
